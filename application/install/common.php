...
function create_tables($db, $prefix = ''){
    // 读取SQL文件
    $sql = file_get_contents(APP_PATH . 'install/data/db.sql');
 ...
 */
function create_default_tenant($db, $db_config){
    // 读取SQL文件
    $sql = 'INSERT INTO `tp_tenants`(`id`, `name`, `dbstr`) VALUES ("1", "DEFAULT", "'.aesEncryption(json_encode($db_config)).'");';
    $db->execute($sql);
}
